...
class NumMatrix:
    ...
    def sumRegion(self, row1: int, col1: int, row2: int, col2: int) -> int:
        result = 0
        row = 0
        while row <= len(self.prefix_matrix) - 1:
            for column in range(len(self.prefix_matrix[0])):
                if row1 <= row <= row2 and col1 <= column <= col2:
                    right_sum = self.prefix_matrix[row][col2]
                    left_sum = self.prefix_matrix[row][col1 - 1] if row > 0 else self.prefix_matrix[row][column]
...
print(solution.sumRegion(1, 2, 2, 4))
